...
import com.biancama.utils.zip.UnZip;
...
public class WebUpdater implements Serializable {
    ...
    public static final String[] UPDATE_MIRROR = EasyShipmentConfigConstants.UPDATE_MIRROR;
    ...
    public static HashMap<String, FileUpdate> getPluginList() {
        if (PLUGIN_LIST == null && URLUtils.getResourceFile("tmp/hashlist.lst").exists()) {
            ...
            updater.parseFileList(URLUtils.getResourceFile("tmp/hashlist.lst"), null, PLUGIN_LIST);
        }
    ...
    private transient BiancaBroadcaster<MessageListener, MessageEvent> broadcaster;
                ...
                if (br.getRequest().getHttpConnection().isOK()) {
                    ...
                    if (branches.length > 0 && savedBranch != null && isBetaBranch(savedBranch)) {
                        ...
                        if (betaBranch == null || !savedBranch.equals(betaBranch)) {
                            ...
                            SubConfiguration.getConfig("WEBUPDATE").save();
                            BiancaLogger.getLogger().severe("RESETTED BRANCH; SINCE BETA branch " + savedBranch + " is not available any more");
                        }
    ...
    private void loadUpdateList() throws Exception {
        for (int trycount = 0; trycount < 10; trycount++) {
            try {
                ...
                String localHash = HashUtils.getMD5(URLUtils.getResourceFile(UPDATE_ZIP_LOCAL_PATH));
                if (!serverHash.equalsIgnoreCase(localHash)) {
                    path = getZipUrl(trycount);
                    if (path == null) {
                        continue;
                    }
                    Browser.download(URLUtils.getResourceFile(UPDATE_ZIP_LOCAL_PATH), path + "?t=" + System.currentTimeMillis());
                }
                UnZip u = new UnZip(URLUtils.getResourceFile(UPDATE_ZIP_LOCAL_PATH), URLUtils.getResourceFile("tmp/"));
    ...
    private void parseFileList(File file, ArrayList<FileUpdate> ret, HashMap<String, FileUpdate> plugins) {
        String source;
        source = BiancaIO.readFileToString(file);
    ...
    public void updateFile(Vector<String> file) throws IOException {
        ...
        broadcaster.fireEvent(new MessageEvent(this, DO_UPDATE_FILE, String.format("Download %s to %s", WebUpdater.formatPathReadable(tmp[1]), WebUpdater.formatPathReadable(URLUtils.getResourceFile(tmp[0]).getAbsolutePath()))));

        Browser.download(URLUtils.getResourceFile(tmp[0]), tmp[0]);
