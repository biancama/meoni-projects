...
public class ShortCuts {
    ...
    private static String getModifiersText(int modifiers) {
        ...
        if ((modifiers & KeyEvent.CTRL_DOWN_MASK) != 0) {
            ...
            buf.append(BiancaL.L(JDL_PREFIX + "key.ctrl", "ctrl"));
        }
        if ((modifiers & KeyEvent.META_DOWN_MASK) != 0) {
            if (buf.length() > 0) {
                buf.append('+');
            }
            buf.append(BiancaL.L(JDL_PREFIX + "key.meta", "meta"));
        }
        if ((modifiers & KeyEvent.ALT_DOWN_MASK) != 0) {
            if (buf.length() > 0) {
                buf.append('+');
            }
            buf.append(BiancaL.L(JDL_PREFIX + "key.alt", "alt"));
        }
        if ((modifiers & KeyEvent.ALT_GRAPH_DOWN_MASK) != 0) {
            if (buf.length() > 0) {
                buf.append('+');
            }
            buf.append(BiancaL.L(JDL_PREFIX + "key.altGr", "alt Gr"));
        }
        if ((modifiers & KeyEvent.SHIFT_DOWN_MASK) != 0) {
            if (buf.length() > 0) {
                buf.append('+');
            }
            buf.append(BiancaL.L(JDL_PREFIX + "key.shift", "shift"));
        }
        if ((modifiers & KeyEvent.BUTTON1_DOWN_MASK) != 0) {
            if (buf.length() > 0) {
                buf.append('+');
            }
            buf.append(BiancaL.L(JDL_PREFIX + "key.button1", "button1"));
        }
        if ((modifiers & KeyEvent.BUTTON2_DOWN_MASK) != 0) {
            if (buf.length() > 0) {
                buf.append('+');
            }
            buf.append(BiancaL.L(JDL_PREFIX + "key.button2", "button2"));
        }
        if ((modifiers & KeyEvent.BUTTON3_DOWN_MASK) != 0) {
            if (buf.length() > 0) {
                buf.append('+');
            }
            buf.append(BiancaL.L(JDL_PREFIX + "key.button3", "button3"));
        }
    ...
    private static String getVKText(int keyCode) {
        ...
        for (int i = 0; i < fields.length; i++) {
            try {
                if (fields[i].getModifiers() == expected_modifiers && fields[i].getType() == Integer.TYPE && fields[i].getName().startsWith("VK_") && fields[i].getInt(KeyEvent.class) == keyCode) { return BiancaL.L(JDL_PREFIX + fields[i].getName(), fields[i].getName().substring(3)); }
