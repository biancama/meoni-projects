...
import com.biancama.gui.swing.userio.DummyFrame;
...
public abstract class AbstractDialog extends JCountdownDialog implements ActionListener {
    ...
    public AbstractDialog(int flag, String title, ImageIcon icon, String okOption, String cancelOption) {
        ...
        this.icon = (FlagsUtils.hasAllFlags(flag, UserIO.NO_ICON)) ? null : icon;
        this.okOption = (okOption == null) ? BiancaL.L("gui.btn_ok", "Ok") : okOption;
        this.cancelOption = (cancelOption == null) ? BiancaL.L("gui.btn_cancel", "Cancel") : cancelOption;
    }

    public void init() {
        dont: if (FlagsUtils.hasAllFlags(flag, UserIO.DONT_SHOW_AGAIN)) {
            ...
            if ((value = cfg.getProperty(getDontShowAgainKey())) != null && value instanceof Integer) {
                ...
                if (FlagsUtils.hasAllFlags(flag, UserIO.DONT_SHOW_AGAIN_IGNORES_CANCEL) && FlagsUtils.hasAllFlags(ret, UserIO.RETURN_CANCEL)) {
                    break dont;
                }
                if (FlagsUtils.hasAllFlags(flag, UserIO.DONT_SHOW_AGAIN_IGNORES_OK) && FlagsUtils.hasAllFlags(ret, UserIO.RETURN_OK)) {
                    break dont;
                }
        ...
        if ((flag & UserIO.DONT_SHOW_AGAIN) > 0) {
            ...
            add(dontlabel = new JLabel(BiancaL.L("gui.dialogs.dontshowthisagain", "Don't show this again")));
        ...
        if (FlagsUtils.hasNoFlags(flag, UserIO.NO_COUNTDOWN)) {
            this.countdown(UserIO.getCountdownTime());
        } else {
            countDownLabel.setVisible(false);
        }
        if (dont != null) {
            btnOK.addMouseListener(new BiancaMouseAdapter() {

                @Override
                public void mouseEntered(MouseEvent e) {
                    if (FlagsUtils.hasAllFlags(flag, UserIO.DONT_SHOW_AGAIN_IGNORES_OK)) {
            ...
            btnCancel.addMouseListener(new BiancaMouseAdapter() {

                @Override
                public void mouseEntered(MouseEvent e) {
                    if (FlagsUtils.hasAllFlags(flag, UserIO.DONT_SHOW_AGAIN_IGNORES_CANCEL)) {
                        dont.setEnabled(false);
                        dontlabel.setEnabled(false);
                    }
        ...
        if (SwingGui.getInstance() == null) {
            this.setLocation(BiancaScreen.getCenterOfComponent(null, this));
    ...
    protected void setReturnValue(boolean b) {
        ...
        if (FlagsUtils.hasAllFlags(flag, UserIO.DONT_SHOW_AGAIN)) {
