...
public class ConvertDialog extends AbstractDialog {
    ...
    private final ArrayList<ConversionMode> modes;
    ...
    private ConvertDialog(ArrayList<ConversionMode> modes, String name) {
        super(UserIO.NO_COUNTDOWN, BiancaL.L("convert.dialog.chooseformat", "Wähle das Dateiformat:") + " [" + name + "]", UserIO.getInstance().getIcon(UserIO.ICON_QUESTION), null, null);
    ...
    public JComponent contentInit() {
        chkKeepFormat = new JCheckBox(ConvertDialog.hasKeeped() ? BiancaL.L("convert.dialog.staykeepingformat", "Formate weiterhin beibehalten") : BiancaL.L("convert.dialog.keepformat", "Format für diese Sitzung beibehalten"));
        chkKeepFormat.setSelected(ConvertDialog.hasKeeped());

        chkForceKeep = new JCheckBox(BiancaL.L("convert.dialog.forcekeep", "Beibehalten erzwingen"));
        chkForceKeep.setSelected(ConvertDialog.forcekeep);

        chkTopPriority = new JCheckBox(BiancaL.L("convert.dialog.toppriority", "Diese Auswahl vorherigen immer vorziehen"));
