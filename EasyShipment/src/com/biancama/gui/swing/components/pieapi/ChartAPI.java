...
package com.biancama.gui.swing.components.pieapi;
...
/**
 * Die ChartAPI greift auf die Google Chart API zurück. Sie funktioniert nur
 * mit einer intakten Internet-Verbindung!
 * 
 * @author gluewurm
 */
public abstract class ChartAPI extends JComponent {
    ...
    private final HashMap<String, ChartAPIEntity> collData = new HashMap<String, ChartAPIEntity>();
    private int width;
    private int height;
    ...
    public long getMaxValue() {
        long max = 0;
        for (ChartAPIEntity tmp : collData.values()) {
            if (Long.valueOf(tmp.getData()) > max) {
    ...
    public void paintComponent(Graphics g) {
        ...
        g.drawString(BiancaL.LF("plugins.config.premium.chartapi.caption.error", "%s Chart is loading or not available", caption), 0, 10);
    }
