...
public class Balloon {
    ...
    public static void main(String args[]) throws InterruptedException {
        Balloon.show("AccountController", BiancaTheme.II("gui.images.accounts", 32, 32), "Premiumaccounts are globally disabled!<br/>Click <a href='http://jdownloader.org/knowledge/wiki/gui/premiummenu'>here</a> for help.");

        Thread.sleep(2 * 1000);

        Balloon.show("title", null, BiancaTheme.II("gui.images.help", 32, 32), "This is <b>just dummy</b><br/> text.<a href='http://www.google.de'>LINK</a> you added 5 links");
    }
    ...
    public static void show(final String title, ImageIcon ii, final JPanel panel) {
        if (!GUIUtils.getConfig().getBooleanProperty(EasyShipmentGuiConstants.PARAM_SHOW_BALLOON.toString(), true)) { return; }

        final ImageIcon icon;
        if (ii == null) {
            icon = new ImageIcon(BiancaImage.getImage("logo/logo_16_16"));
        } else {
            icon = ii;
        }
        ...
        new GuiRunnable<Object>() {
            ...
            public Object runSave() {
                ...
                bt.addMouseListener(new BiancaMouseAdapter() {
                    @Override
                ...
                titlePanel.addMouseListener(new BiancaMouseAdapter() {
                    @Override
                    public void mouseClicked(MouseEvent e) {
    ...
    private static void locate(JWindow w, int y) {
        Point point = BiancaScreen.getDockBottomRight(w);
        point.x -= GAP;
        point.y -= (GAP + y);
        w.setLocation(point);
