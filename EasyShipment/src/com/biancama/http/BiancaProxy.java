...
public class BiancaProxy extends Proxy {
    ...
    public static BiancaProxy NO_PROXY = new BiancaProxy();

    public int getPort() {
        return port;
    }

    public BiancaProxy(java.net.Proxy.Type type, SocketAddress sa) {
        super(type, sa);
        this.host = new Regex(sa.toString(), "(.*)\\/").getMatch(0);
    }

    public BiancaProxy(java.net.Proxy.Type type, String host, int port) {
    ...
    private static String[] getInfo(String host, String port) {
        ...
        if (tmpport != null) {
            ...
            if (tmpport != null) {
                ...
                BiancaLogger.getLogger().severe("No proxyport defined, using default 8080");
                info[1] = "8080";
            }
        }
    ...
    public BiancaProxy(String hostAndPort) {
        super(Type.HTTP, new InetSocketAddress(getInfo(hostAndPort, null)[0], Integer.parseInt(getInfo(hostAndPort, null)[1])));
        port = Integer.parseInt(getInfo(hostAndPort, null)[1]);
        host = getInfo(hostAndPort, null)[0];
    }

    public BiancaProxy(String host, int port, String user, String pass) {
        super(Type.HTTP, new InetSocketAddress(getInfo(host, "" + port)[0], port));
        this.port = port;
        this.user = user;
        this.pass = pass;
        this.host = getInfo(host, "" + port)[0];
    ...
    private BiancaProxy() {
        super(Type.HTTP, new InetSocketAddress(80));
        this.direct = true;
    }
