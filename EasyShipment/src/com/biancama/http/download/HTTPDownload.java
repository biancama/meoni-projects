...
import com.biancama.utils.jobber.Threader;
import com.biancama.utils.jobber.Threader.Worker;
...
public class HTTPDownload extends DownloadInterface {
    ...
    private final Request orgRequest;
    ...
    public static void main(String[] args) {
        try {
            ...
            try {
                ...
                dl.start();
                long crc = URLUtils.getCRC(new File(destPath));
    ...
    private void clean() throws BrowserException {
        if (!new File(getOutputFile().getAbsolutePath() + ".part").renameTo(getOutputFile())) { throw new BrowserException(BiancaL.L("exceptions.browserexception.couldnotrenam", "Could not rename getOutputFile()"), BrowserException.TYPE_LOCAL_IO);
    ...
    private void initOutputChannel() throws FileNotFoundException, BrowserException {
        if (getOutputFile().exists()) { throw new BrowserException(BiancaL.L("exceptions.browserexception.alreadyexists", "getOutputFile() already exists"), BrowserException.TYPE_LOCAL_IO);
    ...
    /**
     * Funktion überprüft ob eventl noch chunks hinzugefügt werden müssen um
     * die datei zu ende zu laden. z.B. wenn chunks abgebrochen wurden.
     * 
     * @throws BrowserException
     */
    ...
    private void initChunks() throws IOException, BrowserException {
        ...
        chunks.getBroadcaster().addListener(chunks.new WorkerListener() {
            ...
            // @Override
            @Override
            public void onThreadException(Threader th, BiancaRunnable job, Exception e) {
            ...
            // @Override
            @Override
            public void onThreadFinished(Threader th, BiancaRunnable job) {
                try {
            ...
            // @Override
            @Override
            public void onThreadStarts(Threader threader, BiancaRunnable runnable) {
                updateActiveChunkCount(+1);
