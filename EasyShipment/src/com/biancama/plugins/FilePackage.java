...
class FilePackageBroadcaster extends BiancaBroadcaster<FilePackageListener, FilePackageEvent> {
...
public class FilePackage extends Property implements Serializable, DownloadLinkListener, FilePackageListener {
    ...
    public synchronized static FilePackage getDefaultFilePackage() {
        if (FP == null) {
            FP = new FilePackage();
            FP.setName(BiancaL.L("controller.packages.defaultname", "various"));
        }
        return FP;
    }
    ...
    private FilePackage() {
        ...
        downloadDirectory = FileSystemUtils.getDefaultDownloadDirectory();
        counter++;
    ...
    public String getDownloadDirectory() {
        return downloadDirectory == null ? FileSystemUtils.getDefaultDownloadDirectory() : downloadDirectory;
    }

    /**
     * @return Gibt nur den namen des Downloadverzeichnisses zurück. ACHTUNG!
     *         es wird nur der Directory-NAME zurückgegeben, nicht der ganze
     *         Pfad
     */
    ...
    public boolean isFinished() {
        if (System.currentTimeMillis() - updateTime1 > UPDATE_INTERVAL) {
                ...
                synchronized (downloadLinkList) {
                    for (DownloadLink lk : downloadLinkList) {
                        if (!lk.getLinkStatus().hasStatus(LinkStatus.FINISHED) && lk.isEnabled()) {
                            ...
                            if (lk.getFinishedDate() != -1) {
                                lastfinished = lastfinished >= lk.getFinishedDate() ? lastfinished : lk.getFinishedDate();
                            }
                        }
                    }
                }
            ...
            if (!isFinished) {
                finishedDate = -1;
            } else if (isFinished && finishedDate == -1) {
                finishedDate = lastfinished;
            }
        }
        return isFinished;
    }
    ...
    /**
     * Gibt die Anzahl der Verbleibenden Links zurück. Wurden alle Links
     * bereits abgearbeitet gibt diese Methode 0 zurück Da die Methode alle
     * Links durchläuft sollte sie aus Performancegründen mit bedacht
     * eingesetzt werden
     */
            ...
            synchronized (downloadLinkList) {
                ...
                if (ret) {
                    ...
                    if (downloadLinkList.size() == 0) {
                        broadcaster.fireEvent(new FilePackageEvent(this, FilePackageEvent.FILEPACKAGE_EMPTY));
                    }
                }
            }
        }
    ...
    public void setDownloadDirectory(String subFolder) {
        downloadDirectory = FileSystemUtils.removeEndingPoints(subFolder);
    }

    public void setName(String name) {
        if (name == null || name.length() == 0) {
            this.name = FileSystemUtils.removeEndingPoints(getDefaultFilePackage().name);
        } else {
            this.name = FileSystemUtils.removeEndingPoints(FileSystemUtils.validateFileandPathName(name));
        }
