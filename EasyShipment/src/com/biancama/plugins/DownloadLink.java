...
class DownloadLinkBroadcaster extends BiancaBroadcaster<DownloadLinkListener, DownloadLinkEvent> {
...
public class DownloadLink extends Property implements Serializable, Comparable<DownloadLink> {
    ...
    private transient static Logger logger = BiancaLogger.getLogger();
    ...
    public synchronized BiancaBroadcaster<DownloadLinkListener, DownloadLinkEvent> getBroadcaster() {
        if (broadcaster == null) {
    ...
    public DownloadLink addSourcePluginPassword(String sourcePluginPassword) {
        ...
        synchronized (sourcePluginPasswordList) {
            if (!sourcePluginPasswordList.contains(sourcePluginPassword)) {
    ...
    public int getDownloadSpeed() {
        ...
        synchronized (this.getDownloadInstance().getChunks()) {
            for (Chunk ch : this.getDownloadInstance().getChunks()) {
                if (ch.inProgress()) {
                    currspeed += ch.getSpeedMeter().getSpeed();
                }
            }
        }
        return currspeed;
        ...
        for (int retry = 0; retry < 5; retry++) {
            try {
                ...
                if (e.getMessage().contains("code: 500")) {
                    try {
                        wait += 500;
                        BiancaLogger.getLogger().finer("500 Error Code, retrying in " + wait);
                        Thread.sleep(wait);
                    ...
                    // BiancaLogger.getLogger().log(java.util.logging.
                ...
                // logger.severe("Hoster Plugin Version: " +
                // getPlugin().getVersion());
                // BiancaLogger.getLogger().log(java.util.logging.Level
    ...
    public void reset() {
        ...
        finalFileName = null;
        if (getPlugin() != null) {
            getPlugin().resetDownloadlink(this);
        }
    }
    ...
    public void deleteFile(boolean partfile, boolean finalfile) {
        ...
        if (finalfile && new File(this.getFileOutput()).exists()) {
            if (!new File(this.getFileOutput()).delete()) {
                logger.severe(BiancaL.L("system.download.errors.couldnotdelete", "Could not delete file") + this.getFileOutput());
            }
        }
        ...
        if (dlFolder != null && dlFolder.exists() && dlFolder.isDirectory() && dlFolder.listFiles() != null && dlFolder.listFiles().length == 0) {
            if (!new File(FileSystemUtils.getDefaultDownloadDirectory()).equals(dlFolder)) {
    ...
    public void addSubdirectory(String subdir) {
        if (subdir != null && name.length() > 0) {
            if (subdirectory != null) {
                subdirectory = new StringBuilder(subdirectory).append(File.separator).append(FileSystemUtils.removeEndingPoints(BiancaIO.validateFileandPathName(subdir))).toString();
            } else {
                subdirectory = FileSystemUtils.removeEndingPoints(BiancaIO.validateFileandPathName(subdir));
            }
        } else {
            subdirectory = null;
        }
    ...
    public void setName(String name) {
        if (name != null && name.length() > 0) {
            this.name = FileSystemUtils.removeEndingPoints(BiancaIO.validateFileandPathName(name));
        }
        this.setIcon(null);
    ...
    public void setFinalFileName(String newfinalFileName) {
        ...
        if (newfinalFileName != null && newfinalFileName.length() > 0) {
            ...
            finalFileName = FileSystemUtils.removeEndingPoints(BiancaIO.validateFileandPathName(newfinalFileName));
        } else {
            finalFileName = null;
        }
    ...
    public ImageIcon getUnscaledIcon() {
        return BiancaImage.getFileIcon(BiancaIO.getFileExtension(getFileOutput()));
    ...
    public ImageIcon getIcon() {
        if (icon == null) {
            if (OSDetector.isLinux() || OSDetector.isMac()) {
                try {
                    Image image = BiancaTheme.getImage(BiancaTheme.getTheme() + "/mime/" + BiancaIO.getFileExtension(this.getName()), 16, 16);
                    icon = new ImageIcon(image);
                } catch (Exception e) {
                    icon = BiancaTheme.II("gui.images.link", 16, 16);
                }
            } else {
                try {
                    icon = BiancaImage.getScaledImageIcon(BiancaImage.getFileIcon(BiancaIO.getFileExtension(getFileOutput())), 16, 16);
                } catch (Exception e) {
                    try {
                        Image image = BiancaTheme.getImage(BiancaTheme.getTheme() + "/mime/" + BiancaIO.getFileExtension(this.getName()), 16, 16);
                        icon = new ImageIcon(image);
    ...
    public static boolean isSpeedLimited() {
        return SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SPEED.toString(), 0) != 0;
    }
