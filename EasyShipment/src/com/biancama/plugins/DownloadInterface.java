...
import com.biancama.controlling.SpeedMeter;
...
import com.biancama.utils.ByteBufferController;
import com.biancama.utils.ByteBufferEntry;
...
abstract public class DownloadInterface {
    ...
    public class Chunk extends Thread {
        ...
        private final DownloadInterface dl;
        ...
        private final SpeedMeter speed = new SpeedMeter();
        ...
        public void checkTimeout(long timeout) {
            ...
            if (dif >= timeout) {
                ...
                downloadLink.getLinkStatus().setStatusText(BiancaL.L("download.connection.idle", "Idle"));
        ...
        private URLConnectionAdapter copyConnection(URLConnectionAdapter connection) {
            ...
            try {
                ...
                addException(e);
                error(LinkStatus.ERROR_RETRY, FormatterUtils.convertExceptionReadable(e));
        ...
        private void download() {
            ...
            try {
                ...
                error(LinkStatus.ERROR_FATAL, BiancaL.L("download.error.message.outofmemory", "The downloadsystem is out of memory"));
            ...
            try {
                ...
                if (getCurrentBytesPosition() < endByte && endByte > 0 || getCurrentBytesPosition() <= 0) {
                    ...
                    error(LinkStatus.ERROR_DOWNLOAD_FAILED, BiancaL.L("download.error.message.incomplete", "Download unvollständig"));
                }
                inputStream.close();
                if (source.isOpen()) {
                    source.close();
                ...
                error(LinkStatus.ERROR_LOCAL_IO, BiancaL.L("download.error.message.iopermissions", "No permissions to write to harddisk"));
            } catch (UnknownHostException e) {
                linkStatus.setValue(10 * 60000l);
                error(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, BiancaL.L("download.error.message.unavailable", "Service temp. unavailable"));
            } catch (IOException e) {
        ...
        /**
         * Gibt die Aktuelle Endposition in der gesamtfile zurück. Diese
         * Methode gibt die Endposition unahängig davon an Ob der aktuelle
        ...
        /**
         * Gibt eine Abschätzung zurück wie schnell der Chunk laden könnte
         * wenn man ihn nicht bremsen würde.
         * 
         * @return
         */
        ...
        /**
         * Gibt die geladenen Partbytes zurück. Das entsüricht bei resumen
            ...
            try {
                ...
                if (chunkNum > 1) {
                    ...
                    if (connection == null) {
                        ...
                        error(LinkStatus.ERROR_DOWNLOAD_FAILED, BiancaL.L("download.error.message.connectioncopyerror", "Could not clone the connection"));
                    ...
                    if (startByte > 0 && (connection.getHeaderField("Content-Range") == null || connection.getHeaderField("Content-Range").length() == 0)) {
                        error(LinkStatus.ERROR_DOWNLOAD_FAILED, BiancaL.L("download.error.message.rangeheaders", "Server does not support chunkload"));
                ...
                if (startByte > 0) {
                    ...
                    if (range == null && chunkNum > 1) {
                        if (dl.fakeContentRangeHeader()) {
                            ...
                            long gotSB = FormatterUtils.filterLong(fixrange[0][0]);
                            long gotEB;
                            if (fixrange[0][1] == null) {
                                gotEB = FormatterUtils.filterLong(fixrange[0][0]) + connection.getLongContentLength() - 1;
                            } else {
                                gotEB = FormatterUtils.filterLong(fixrange[0][1]);
                            }
                            if (gotSB != startByte) {
                            ...
                            error(LinkStatus.ERROR_DOWNLOAD_FAILED, BiancaL.L("download.error.message.rangeheaderparseerror", "Unexpected rangeheader format:") + connection.getHeaderField("Content-Range"));
                        ...
                        long gotSB = FormatterUtils.filterLong(range[0][0]);
                        long gotEB = FormatterUtils.filterLong(range[0][1]);
                        long gotS = FormatterUtils.filterLong(range[0][2]);
                        if (gotSB != startByte) {
    ...
    public static Logger logger = BiancaLogger.getLogger();
    ...
    private final Vector<Integer> errors = new Vector<Integer>();
    ...
    public long head() throws IOException, PluginException {
        ...
        if (head.getContentLength() > 1024) {
            ...
            if (name != null) {
                this.downloadLink.setName(name);
            }
    ...
    public boolean handleErrors() {
        ...
        if (this.doFileSizeCheck && (totaleLinkBytesLoaded <= 0 || totaleLinkBytesLoaded != fileSize && fileSize > 0)) {
            ...
            error(LinkStatus.ERROR_DOWNLOAD_INCOMPLETE, BiancaL.L("download.error.message.incomplete", "Download unvollständig"));
            return false;
        }

        if (getExceptions() != null && getExceptions().size() > 0) {
            error(LinkStatus.ERROR_RETRY, BiancaL.L("download.error.message.incomplete", "Download unvollständig"));
            return false;
        }
        if (!linkStatus.isFailed()) {
            linkStatus.setStatus(LinkStatus.FINISHED);
    ...
    public static boolean preDownloadCheckFailed(DownloadLink link) {
        ...
        DownloadLink block = EventUtils.getDownloadController().getFirstLinkThatBlocks(downloadLink);
        LinkStatus linkstatus = link.getLinkStatus();
        if (block != null) {
            logger.severe("File already is in progress. " + downloadLink.getFileOutput());
            linkstatus.addStatus(LinkStatus.ERROR_ALREADYEXISTS);
            linkstatus.setStatusText(BiancaL.LF("system.download.errors.linkisBlocked", "Mirror %s is loading", block.getPlugin().getHost()));
            return true;
        }
        File fileOutput = new File(downloadLink.getFileOutput());
        if (fileOutput.getParentFile() == null) {
            linkstatus.addStatus(LinkStatus.ERROR_FATAL);
            linkstatus.setErrorMessage(BiancaL.L("system.download.errors.invalidoutputfile", "Invalid Outputfile"));
            return true;
        }
        ...
        if (fileOutput.exists()) {
            if (SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_FILE_EXISTS.toString(), 1) == 0) {
                if (!new File(downloadLink.getFileOutput()).delete()) {
                    linkstatus.addStatus(LinkStatus.ERROR_FATAL);
                    linkstatus.setErrorMessage(BiancaL.L("system.download.errors.couldnotoverwrite", "Could not overwrite existing file"));
                    return true;
                }
            } else {
                linkstatus.addStatus(LinkStatus.ERROR_ALREADYEXISTS);
                linkstatus.setErrorMessage(BiancaL.L("downloadlink.status.error.file_exists", "File exists"));
                return true;
            }
        }
        return false;
    ...
    public boolean startDownload() throws Exception {
        ...
        try {
            ...
            if (e instanceof FileNotFoundException) {
                this.error(LinkStatus.ERROR_LOCAL_IO, BiancaL.LF("download.error.message.localio", "Could not write to file: %s", e.getMessage()));
            } else {
                BiancaLogger.exception(e);
            }
            handleErrors();
