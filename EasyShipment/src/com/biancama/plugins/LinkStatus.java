...
public class LinkStatus implements Serializable {
    ...
    /**
     * Downloadinterface: Zeigt an dass der Link nicht vollständig geladen
     * wurde
     */
    ...
    public void addStatus(int status) {
        this.status |= status;
        if (FlagsUtils.hasSomeFlags(status, FINISHED)) {
            if (downloadLink.getFinishedDate() == -1l) {
    ...
    public void exceptionToErrorMessage(Exception e) {
        setErrorMessage(FormatterUtils.convertExceptionReadable(e));
    }

    private String getDefaultErrorMessage() {
        switch (lastestStatus) {
        case LinkStatus.ERROR_RETRY:
            return BiancaL.L("downloadlink.status.error.retry", "Unknown error, retrying");
        case LinkStatus.ERROR_PLUGIN_DEFECT:
            return BiancaL.L("downloadlink.status.error.defect", "Plugin outdated");
        case LinkStatus.ERROR_PLUGIN_NEEDED:
            return BiancaL.L("downloadlink.status.error.no_plugin_available", "No plugin available");
        case LinkStatus.ERROR_DOWNLOAD_INCOMPLETE:
            return BiancaL.L("downloadlink.status.incomplete", "Incomplete");
        case LinkStatus.ERROR_AGB_NOT_SIGNED:
            return BiancaL.L("downloadlink.status.error.agb_not_signed", "TOCs not signed");
        case LinkStatus.ERROR_ALREADYEXISTS:
            return BiancaL.L("downloadlink.status.error.file_exists", "File exists");
        case LinkStatus.ERROR_CAPTCHA:
            return BiancaL.L("downloadlink.status.error.captcha_wrong", "Captcha wrong");
        case LinkStatus.ERROR_DOWNLOAD_FAILED:
            return BiancaL.L("downloadlink.status.error.downloadfailed", "Download failed");
        case LinkStatus.ERROR_IP_BLOCKED:
            return BiancaL.L("downloadlink.status.error.download_limit", "Download Limit reached");
        case LinkStatus.ERROR_FILE_NOT_FOUND:
            return BiancaL.L("downloadlink.status.error.file_not_found", "File not found");
        case LinkStatus.ERROR_POST_PROCESS:
            return BiancaL.L("downloadlink.status.error.post_process", "Processing error");
        case LinkStatus.ERROR_TIMEOUT_REACHED:
        case LinkStatus.ERROR_NO_CONNECTION:
            return BiancaL.L("downloadlink.status.error.no_connection", "No Connection");
        case LinkStatus.ERROR_PREMIUM:
            return BiancaL.L("downloadlink.status.error.premium", "Premium Error");
        case LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE:
            return BiancaL.L("downloadlink.status.error.temp_unavailable", "Temp. unavailable");
        case LinkStatus.ERROR_HOSTER_TEMPORARILY_UNAVAILABLE:
            return BiancaL.L("downloadlink.status.error.hoster_temp_unavailable", "Download from this host is currently not possible");
        case LinkStatus.ERROR_FATAL:
            return BiancaL.L("downloadlink.status.error.fatal", "Fatal Error");
        case LinkStatus.WAITING_USERIO:
            return BiancaL.L("downloadlink.status.waitinguserio", "Waiting for user input");
        }
        return null;
    }
    ...
    public String getLongErrorMessage() {
        ...
        if (ret == null) {
            ret = BiancaL.L("downloadlink.status.error_unexpected", "Unexpected Error");
        }
        return ret;
    }
    ...
    public String getStatusString() {
        ...
        if (hasStatus(LinkStatus.ERROR_POST_PROCESS)) {
            if (getErrorMessage() != null) {
                ...
                ret.append(BiancaL.L("gui.downloadlink.errorpostprocess3", "[convert failed]"));
            }
            return ret.toString();
        }
        ...
        if (!downloadLink.isEnabled() && !hasStatus(LinkStatus.FINISHED)) {
            if (downloadLink.isAborted() && (statusText == null || statusText.trim().length() == 0)) {
                ret.append(BiancaL.L("gui.downloadlink.aborted", "[interrupted]")).append(' ');
        ...
        if (hasStatus(ERROR_IP_BLOCKED) && DownloadWatchDog.getInstance().getRemainingIPBlockWaittime(downloadLink.getHost()) > 0) {
            ret.append(BiancaL.LF("gui.download.waittime_status2", "Wait %s", FormatterUtils.formatSeconds(getRemainingWaittime() / 1000)));
        ...
        if (downloadLink.getPlugin() != null && DownloadWatchDog.getInstance().getRemainingIPBlockWaittime(downloadLink.getHost()) > 0 && !downloadLink.getLinkStatus().isPluginActive()) { return BiancaL.L("gui.downloadlink.hosterwaittime", "[wait for new ip]"); }
        if (downloadLink.getPlugin() != null && DownloadWatchDog.getInstance().getRemainingTempUnavailWaittime(downloadLink.getHost()) > 0 && !downloadLink.getLinkStatus().isPluginActive()) { return BiancaL.L("gui.downloadlink.hostertempunavail", "[download currently not possible]"); }
        ...
        if (hasStatus(LinkStatus.DOWNLOADINTERFACE_IN_PROGRESS)) {
            ...
            if (speed > 0) {
                if (downloadLink.getDownloadSize() < 0) {
                    return FormatterUtils.formatReadable(speed) + "/s " + BiancaL.L("gui.download.filesize_unknown", "(Filesize unknown)");
                } else {
                    long remainingBytes = downloadLink.getDownloadSize() - downloadLink.getDownloadCurrent();
                    long eta = remainingBytes / speed;
                    return "ETA " + FormatterUtils.formatSeconds((int) eta) + " @ " + FormatterUtils.formatReadable(speed) + "/s " + chunkString;
                }
            } else {
                return BiancaL.L("gui.download.create_connection", "Connecting...") + chunkString;
            }
        }

        if (downloadLink.isAvailabilityStatusChecked() && !downloadLink.isAvailable()) { return BiancaL.L("gui.download.onlinecheckfailed", "[Not available]"); }
    ...
    public String toString() {
        ...
        sb.append(FormatterUtils.fillString(Integer.toBinaryString(status), "0", "", 32) + " <Statuscode\r\n");
        String latest = "";
        for (Field field : fields) {
            if (field.getModifiers() == 25) {
                int value;
                try {
                    ...
                    if (hasStatus(value)) {
                        ...
                        sb.append(FormatterUtils.fillString(Integer.toBinaryString(value), "0", "", 32) + " |" + field.getName() + "\r\n");
                    }
                } catch (IllegalArgumentException e) {
                    BiancaLogger.exception(e);
                } catch (IllegalAccessException e) {
    ...
    public void resetStatus(int... notResetifFlag) {
        ...
        // filter flags
        curState = FlagsUtils.filterFlags(curState, resetFlag | LinkStatus.TODO);
        curLState = FlagsUtils.filterFlags(curLState, resetFlag | LinkStatus.TODO);
