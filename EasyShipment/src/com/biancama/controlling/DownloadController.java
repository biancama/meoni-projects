...
import com.biancama.plugins.FilePackageEvent;
import com.biancama.plugins.FilePackageListener;
...
class DownloadControllerBroadcaster extends BiancaBroadcaster<DownloadControllerListener, DownloadControllerEvent> {
...
public class DownloadController implements FilePackageListener, DownloadControllerListener, ActionListener {
    ...
    private final BiancaController controller;

    // private Optimizer optimizer;

    private transient final DownloadControllerBroadcaster broadcaster = new DownloadControllerBroadcaster();
    ...
    private DownloadController() {
        ...
        controller = EventUtils.getController();
        initDownloadLinks();
    ...
    private void initDownloadLinks() {
        ...
        if (packages == null) {
            ...
            File file = URLUtils.getResourceFile("backup/links.linkbackup");
            if (file.exists()) {
    ...
    public void saveDownloadLinksSyncnonThread() {
        ...
        String id = BiancaController.requestDelayExit("downloadcontroller");
        synchronized (packages) {
            saveinprogress = true;
            DatabaseUtils.getDatabaseConnector().saveLinks(packages);
    ...
    private ArrayList<FilePackage> loadDownloadLinks() throws Exception {
        Object obj = DatabaseUtils.getDatabaseConnector().getLinks();
        if (obj != null && obj instanceof ArrayList && (((ArrayList) obj).size() == 0 || ((ArrayList) obj).size() > 0 && ((ArrayList) obj).get(0) instanceof FilePackage)) {
            ...
            while (iterator.hasNext()) {
                ...
                while (it.hasNext()) {
                    ...
                    if (localLink.getLinkStatus().isFinished() && ApplicationUtils.getConfiguration().getIntegerProperty(Configuration.Param.PARAM_FINISHED_DOWNLOADS_ACTION.toString(), 3) == 1) {
                        it.remove();
                        if (fp.getDownloadLinkList().size() == 0) {
                            iterator.remove();
                            continue;
                        ...
                        try {
                            pluginForHost = ApplicationUtils.getNewPluginForHostInstance(localLink.getHost());
                        } catch (Exception e) {
                        ...
                        try {
                            if (localLink.getContainer() != null) {
                                pluginForContainer = ApplicationUtils.getPluginForContainer(localLink.getContainer(), localLink.getContainerFile());
            ...
            while (iterator.hasNext()) {
                ...
                while (it.hasNext()) {
                    ...
                    if (localLink.getLinkStatus().isFinished() && FileSystemUtils.getConfiguration().getIntegerProperty(Configuration.Param.PARAM_FINISHED_DOWNLOADS_ACTION.toString()) == 1) {
                        it.remove();
                        if (fp.getDownloadLinkList().size() == 0) {
                            convert.remove(fp);
                            continue;
