...
import com.biancama.DecryptPluginWrapper;
...
import com.biancama.parser.html.HTMLParser;
...
import com.biancama.utils.encoding.HTMLEntities;
...
/**
 * Diese Klasse läuft in einem Thread und verteilt den Inhalt der
 * Zwischenablage an (unter Umständen auch mehrere) Plugins Die gefundenen
...
public class DistributeData extends Thread {
    ...
    private final ArrayList<String> foundPasswords;
    ...
    public ArrayList<DownloadLink> findLinks() {
        ...
        if (ret != null && ret.size() == 1) {
            ...
            if (!ret.get(0).getPlugin().getWrapper().isEnabled()) {
                ret.clear();
            } else {
                ...
                if (pHost.canHandle(decrypted.getDownloadURL())) {
                    ...
                    for (int c = 0; c < dLinks.size(); c++) {
                        ...
                        if (decrypted.isAvailabilityStatusChecked()) {
                            dLinks.get(c).setAvailable(decrypted.isAvailable());
                        }
    ...
    public void run() {
        ...
        if (links.size() == 0 && !disableDeepEmergencyScan) {
            ...
            if (ls.length > 0) {
                ...
                String title = BiancaL.L("gui.dialog.deepdecrypt.title", "Deep decryption?");
                String message = BiancaL.LF("gui.dialog.deepdecrypt.message", "JDownloader has not found anything on %s\r\n-------------------------------\r\nJD now loads this page to look for further links.", txt + "");
                int res = UserIO.getInstance().requestConfirmDialog(0, title, message, BiancaTheme.II("gui.images.search", 32, 32), BiancaL.L("gui.btn_continue", "Continue"), null);
                if (FlagsUtils.hasAllFlags(res, UserIO.RETURN_OK)) {
    ...
    private static String getLoadLinkString(String linkstring) {
        ...
        ProgressController pc = new ProgressController(BiancaL.LF("gui.addurls.progress", "Parse %s URL(s)", links.length), links.length);
        int i = 0;
        ...
        for (String l : links) {
            ...
            try {
                new URL(l);
                pc.setStatusText(BiancaL.LF("gui.addurls.progress.get", "Parse %s URL(s). Get %s links", links.length, l));
            ...
            pc.setStatusText(BiancaL.LF("gui.addurls.progress.found", "Parse %s URL(s). Found %s links", links.length, i));
            pc.increase(1);

        }
        BiancaLogger.getLogger().info("Found Links" + sb);
        pc.doFinalize(2000);
        return sb.toString();
    }
}