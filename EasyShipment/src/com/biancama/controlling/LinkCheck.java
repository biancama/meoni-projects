...
class LinkCheckBroadcaster extends BiancaBroadcaster<LinkCheckListener, LinkCheckEvent> {
...
public class LinkCheck implements ActionListener, ProgressControllerListener {
    ...
    private final ArrayList<DownloadLink> linksToCheck = new ArrayList<DownloadLink>();
    ...
    public synchronized BiancaBroadcaster<LinkCheckListener, LinkCheckEvent> getBroadcaster() {
        if (broadcaster == null) {
            broadcaster = new LinkCheckBroadcaster();
        }
        ...
        for (DownloadLink element : links) {
            synchronized (linksToCheck) {
                if (!linksToCheck.contains(element)) {
                    if (resetLinkCheck) {
                        element.setAvailableStatus(DownloadLink.AvailableStatus.UNCHECKED);
                    }
    ...
    private void startLinkCheck() {
        ...
        checkThread = new Thread() {
            ...
            public void run() {
                ...
                pc = new ProgressController(BiancaL.L("gui.linkgrabber.pc.onlinecheck", "Checking online availability..."));
    ...
    class CheckThread implements BiancaRunnable {
