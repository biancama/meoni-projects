...
public class HTTPLiveHeader extends ReconnectMethod {
    ...
    protected boolean runCommands(ProgressController progress) {
        ...
        if (configuration.getIntegerProperty(ReconnectMethod.PARAM_RECONNECT_TYPE, ReconnectMethod.LIVEHEADER) == ReconnectMethod.CLR) {
            ...
            String[] ret = CLRLoader.createLiveHeader(configuration.getStringProperty(Configuration.Param.PARAM_HTTPSEND_REQUESTS_CLR.toString()));
            if (ret != null) {
                script = ret[1];
            } else {
                script = null;
            ...
            script = configuration.getStringProperty(Configuration.Param.PARAM_HTTPSEND_REQUESTS.toString());
        }
        String user = configuration.getStringProperty(Configuration.Param.PARAM_HTTPSEND_USER.toString());
        String pass = configuration.getStringProperty(Configuration.Param.PARAM_HTTPSEND_PASS.toString());
        String ip = configuration.getStringProperty(Configuration.Param.PARAM_HTTPSEND_IP.toString());
        ...
        br.setProxy(BiancaProxy.NO_PROXY);
        if (user != null && pass != null) {
            br.setAuth(ip, user, pass);
        }
        try {
            xmlScript = ParserUtils.parseXmlString(script, false);
            if (xmlScript == null) {
                progress.doFinalize();
            ...
            for (int step = 0; step < steps.getLength(); step++) {
                progress.setStatusText(BiancaL.L("interaction.liveHeader.progress.3_step", "(STEP)HTTPLiveHeader :") + step);
                progress.increase(1);
                ...
                for (int toDoStep = 0; toDoStep < toDos.getLength(); toDoStep++) {
                    ...
                    progress.setStatusText(BiancaL.LF("interaction.liveHeader.progress.4_step", "(%s)HTTPLiveHeader", toDo.getNodeName()));
                    ...
                    if (toDo.getNodeName().equalsIgnoreCase("WAIT")) {
                        ...
                        int seconds = FormatterUtils.filterInt(item.getNodeValue());
    ...
    public static ArrayList<String[]> getLHScripts() {
        File[] list = new File(new File(FileSystemUtils.getHomeDirectoryFromEnvironment(), "jd"), "router").listFiles();
        ArrayList<String[]> ret = new ArrayList<String[]>();
        for (File element : list) {
            if (element.isFile() && element.getName().toLowerCase().matches(".*\\.xml$")) {
                ret.addAll((Collection<? extends String[]>) FileSystemUtils.loadObject(new JFrame(), element, true));
            }
        }
