...
import com.biancama.controlling.reconnect.Reconnecter;
...
public class DownloadWatchDog implements ControlListener, DownloadControllerListener {
    ...
    private static final String BiancaL_PREFIX = "jd.controlling.DownloadWatchDog.";
    ...
    public void setStopMark(Object entry) {
        synchronized (stopMark) {
            ...
            if (entry instanceof DownloadLink) {
                ...
                ActionController.getToolBarAction("toolbar.control.stopmark").setToolTipText(BiancaL.LF(BiancaL_PREFIX + "stopmark.downloadlink", "Stopmark is set on Downloadlink: %s", ((DownloadLink) entry).getName()));
                DownloadController.getInstance().fireDownloadLinkUpdate(entry);
            } else if (entry instanceof FilePackage) {
                ActionController.getToolBarAction("toolbar.control.stopmark").setSelected(true);
                ActionController.getToolBarAction("toolbar.control.stopmark").setEnabled(true);
                ActionController.getToolBarAction("toolbar.control.stopmark").setIcon("gui.images.stopmark.enabled");
                ActionController.getToolBarAction("toolbar.control.stopmark").setToolTipText(BiancaL.LF(BiancaL_PREFIX + "stopmark.filepackage", "Stopmark is set on Filepackage: %s", ((FilePackage) entry).getName()));
                DownloadController.getInstance().fireDownloadLinkUpdate(((FilePackage) entry).get(0));
            } else if (entry == hiddenstopMark) {
                ActionController.getToolBarAction("toolbar.control.stopmark").setSelected(true);
                ActionController.getToolBarAction("toolbar.control.stopmark").setEnabled(true);
                ActionController.getToolBarAction("toolbar.control.stopmark").setIcon("gui.images.stopmark.enabled");
                ActionController.getToolBarAction("toolbar.control.stopmark").setToolTipText(BiancaL.L(BiancaL_PREFIX + "stopmark.set", "Stopmark is still set!"));
                ...
                ActionController.getToolBarAction("toolbar.control.stopmark").setToolTipText(BiancaL.L("jd.gui.swing.jdgui.actions.actioncontroller.toolbar.control.stopmark.tooltip", "Stop after current Downloads"));
            }
        ...
        synchronized (StartStopSync) {
            if (downloadStatus == STATE.NOT_RUNNING) {
                ...
                if (SubConfiguration.getConfig("DOWNLOAD").getProperty("MAXSPEEDBEFOREPAUSE", null) != null) {
                    ...
                    SubConfiguration.getConfig("DOWNLOAD").setProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SPEED.toString(), SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty("MAXSPEEDBEFOREPAUSE", 0));
                ...
                BiancaController.getInstance().fireControlEvent(new ControlEvent(this, ControlEvent.CONTROL_DOWNLOAD_START, this));
    ...
    public boolean stopDownloads() {
        ...
        synchronized (StartStopSync) {
            if (downloadStatus == STATE.RUNNING) {
                ...
                BiancaController.getInstance().fireControlEvent(new ControlEvent(this, ControlEvent.CONTROL_DOWNLOAD_STOP, this));
                return true;
            } else {
                return false;
            }
                            ...
                            if (!isDownloadLinkActive(nextDownloadLink)) {
                                if (!nextDownloadLink.getLinkStatus().isPluginActive()) {
                                    if (nextDownloadLink.getLinkStatus().isStatus(LinkStatus.TODO)) {
                                        ...
                                        if (activeDownloadsbyHosts(nextDownloadLink.getPlugin()) < (nextDownloadLink.getPlugin()).getMaxSimultanFreeDownloadNum() && activeDownloadsbyHosts(nextDownloadLink.getPlugin()) < maxPerHost && nextDownloadLink.getPlugin().getWrapper().isEnabled()) {
                                            if (returnDownloadLink == null) {
                                                returnDownloadLink = nextDownloadLink;
                                            } else {
                                                if (nextDownloadLink.getPriority() > returnDownloadLink.getPriority()) {
                                                    returnDownloadLink = nextDownloadLink;
                                                }
                                            }
                                        }
        ...
        synchronized (DownloadLOCK) {
            for (DownloadLink link : links) {
                if (!link.getLinkStatus().hasStatus(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE)) {
                    if ((getRemainingIPBlockWaittime(link.getHost()) <= 0 && getRemainingTempUnavailWaittime(link.getHost()) <= 0)) {
                        if (!isDownloadLinkActive(link)) {
                            if (!link.getLinkStatus().isPluginActive()) {
                                if (link.getLinkStatus().isStatus(LinkStatus.TODO)) {
                                    ...
                                    if (activePerHost < (link.getPlugin()).getMaxSimultanFreeDownloadNum() && link.getPlugin().getWrapper().isEnabled()) {
                                        if (!link.isEnabled()) {
    ...
    public int getSimultanDownloadNum() {
        return SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SIMULTAN.toString(), 2);
    }
    ...
    public int getSimultanDownloadNumPerHost() {
        if (SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SIMULTAN_PER_HOST.toString(), 0) == 0) { return Integer.MAX_VALUE; }
        return SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SIMULTAN_PER_HOST.toString(), 0);
    }
    ...
    public void pauseDownloads(boolean value) {
        ...
        if (value) {
            ...
            SubConfiguration.getConfig("DOWNLOAD").setProperty("MAXSPEEDBEFOREPAUSE", SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SPEED.toString(), 0));
            SubConfiguration.getConfig("DOWNLOAD").setProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SPEED.toString(), SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_DOWNLOAD_PAUSE_SPEED.toString(), 10));
            logger.info("Pause enabled: Reducing downloadspeed to " + SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_DOWNLOAD_PAUSE_SPEED.toString(), 10) + " kb/s");
        } else {
            SubConfiguration.getConfig("DOWNLOAD").setProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SPEED.toString(), SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty("MAXSPEEDBEFOREPAUSE", 0));
            SubConfiguration.getConfig("DOWNLOAD").setProperty("MAXSPEEDBEFOREPAUSE", null);
    ...
    private synchronized void startWatchDogThread() {
        if (this.watchDogThread == null || !this.watchDogThread.isAlive()) {
            ...
            watchDogThread = new Thread() {
                @Override
                public void run() {
                    EventUtils.getController().addControlListener(INSTANCE);
                        ...
                        try {
                            ...
                            if (inProgress > 0) {
                                ...
                                for (FilePackage filePackage : fps) {
                                    ...
                                    int maxspeed = SubConfiguration.getConfig("DOWNLOAD").getIntegerProperty(Configuration.Param.PARAM_DOWNLOAD_MAX_SPEED.toString(), 0) * 1024;
                                    if (maxspeed == 0) {
                    ...
                    EventUtils.getController().fireControlEvent(new ControlEvent(this, ControlEvent.CONTROL_ALL_DOWNLOADS_FINISHED, this));
                    EventUtils.getController().removeControlListener(INSTANCE);
        ...
        synchronized (stopMark) {
            ...
            if (stopMark instanceof FilePackage) {
                for (DownloadLink dl : ((FilePackage) stopMark).getDownloadLinkList()) {
                    if (stopMarkTracker.contains(dl)) {
                        continue;
                    }
    ...
    private void startDownloadThread(DownloadLink dlink) {
        ...
        if (!stopMarkTracker.contains(dlink)) {
            stopMarkTracker.add(dlink);
        }
        download.start();
        ...
        //        	
        // String title =
        // BiancaL.L("system.download.errors.notenoughfreespace.title",
        // "There is not enough free space on '" + outputFolder + "'");
        // String message =
        // BiancaL.L("system.download.errors.notenoughfreespace.message",
